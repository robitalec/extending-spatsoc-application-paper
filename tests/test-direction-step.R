...
sub_DT <- DT[run_grp ==
               DT[diff_tg == 1][, .N, .(run_grp, ID)][N > 4, sample(run_grp, 1)]]
g <- ggplot(sub_DT,
            aes(X, Y, label = round(set_units(direction, 'degree')))) +
  geom_path(arrow = arrow()) +
  geom_point() +
  geom_text(nudge_x = 10, nudge_y = 3, data = sub_DT[timegroup != max(timegroup)]) +
  theme_bw() +
  labs(x = '', y = '') +
