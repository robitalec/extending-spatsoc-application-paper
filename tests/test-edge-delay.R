...
g_delay <- ggplot(delay_test) +
  geom_point(
    aes(timegroup, interaction(ID1, ID2), color = dir_corr_delay),
    size = 5
  ) +
...
sub_DT <- DT[
  timegroup %in%
    delay[fusionID == sub_fusionID, timegroup] &
    ID %in% delay[fusionID == sub_fusionID, c(ID1, ID2)]
]
sub_delay <- delay[fusionID == sub_fusionID]

g <- ggplot(sub_DT, aes(X, Y, color = ID)) +
  geom_path() +
  geom_label(aes(label = timegroup), data = sub_DT) +
  theme_bw()
g2 <- ggplot(sub_delay) +
  geom_point(
    aes(timegroup, interaction(ID1, ID2), color = factor(dir_corr_delay)),
    size = 5
  ) +
