...
group_leader(
  DT_test,
  direction_group = 'group_mean_bearing',
  coords = coords,
  return_rank = TRUE
)
print(DT_test)
...
print(DT_fogo[
  group == DT_fogo[, .N, group][N > 3, sample(group, 1)],
  .(
    id,
    timegroup,
    group,
...
g <- ggplot(DT_test, aes(x, y, color = id)) +
  ...
  geom_text(
    aes(
      label = paste0(
        format(dist_along_group_bearing, digits = 1),
        ' (',
        rank_dist_along_group_bearing,
        ')'
      )
...
intercept_fogo <- sub_fogo[
  1,
  group_mean_y_proj - slope_fogo * group_mean_x_proj
]
intercept_inv_fogo <- sub_fogo[
  1,
  group_mean_y_proj - (-1 / slope_fogo * group_mean_x_proj)
]
...
g_fogo_dist <- ggplot(DT_fogo[N_by_group > 1]) +
  ...
  labs(
    x = 'Distance along group bearing',
    y = 'Rank distance along group bearing'
  ) +
  theme_bw()
