...
#| eval: false
#| echo: true
centroid_group(DT, coords = coords)
```
...
DT <- group_pts(DT, threshold = spatial_threshold, id = id, 
                coords = coords, timegroup = 'timegroup')
cent <- centroid_group(DT, coords = coords)
tinytable::tt(cent[1:5, .(ID, X, Y, datetime, timegroup, group, centroid_X, centroid_Y)])
```
...
#| echo: true
#| eval: false
distance_to_centroid(DT, coords, return_rank = TRUE)
direction_to_centroid(DT, coords)
```
...
#| eval: true
#| warning: false
dist_dir_cent <- distance_to_centroid(cent, coords, return_rank = TRUE)
dist_dir_cent <- direction_to_centroid(cent, coords)
...
leader_direction_group(DT, coords = coords, return_rank = TRUE)
```
...
slope <- sub_DT[1, tan(group_direction)]
...
g_pos <- ggplot(sub_DT, aes(X, Y, color = id)) +
  geom_point(size = 0.8) +
  geom_text(aes(label = paste0(format(dist_along_direction_group, digits = 1),
                               ' (', rank_dist_along_direction_group, ')')),
                nudge_y = 0.5) +
...
g_hist <- ggplot(DT[N_by_group > 1]) +
  geom_histogram(aes(dist_along_direction_group), binwidth = 1) +
  labs(x = 'Distance along group az', y = '') +
  theme_bw()

g_hist2 <- ggplot(DT[N_by_group > 1]) +
  geom_histogram(aes(rank_dist_along_direction_group), binwidth = 1) +
  labs(x = 'Rank distance along group az', y = '') +
  theme_bw()

g_dist <- ggplot(DT[N_by_group > 1]) +
  stat_halfeye(aes(dist_along_direction_group, factor(rank_dist_along_direction_group))) +
...
direction_to_leader(DT, coords = coords)
```
...
DT <- direction_to_leader(DT, coords = coords)

sel_group <- DT[, .N, group][N > 3, sample(group, 1)]
sub <- DT[group == sel_group]
slope <- sub[1, tan(group_direction)]
intercept <- sub[1, centroid_Y - slope * centroid_X]
intercept_inv <- sub[1, centroid_Y - (-1/slope * centroid_X)]

g <- ggplot(sub, aes(X, Y, color = id)) +
  geom_point(size = 0.8) +
  geom_text(aes(label = rank_dist_along_direction_group), nudge_y = 0.5) +
  geom_text(aes(label = paste0(format(dist_leader, digits = 2),
                               ', ',
                               format(direction_leader, digits = 2),
                               ' rad')), nudge_y = -0.5) +
  geom_point(color = 'black', aes(centroid_X, centroid_Y)) +
  geom_abline(slope = slope, intercept = intercept) +
  geom_abline(slope = -1/slope, intercept = intercept_inv,
...
g_dist <- ggplot(DT, aes(direction_leader, 
                         factor(rank_dist_along_direction_group))) +
  stat_halfeye() +
  labs(x = 'Direction to leader', y = 'Rank along group az') +
  theme_bw()

g_dir <- ggplot(DT, aes(dist_leader, factor(rank_dist_along_direction_group))) +
  stat_halfeye() +
...
#| eval: false
# TODO: fix
directional_align <- edge_alignment(DT, threshold = NULL, id = id,
                             timegroup = 'timegroup',
...
(TODO)
<!-- TODO: interindividual difference in direction code and example -->
