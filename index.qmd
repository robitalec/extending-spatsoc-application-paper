...
focused on metrics applicable to spatiotemporal data such as animal movement data.
See details in Supplement A: Scoping Review Methods.
...
distance based edge-list generation and data-stream permutations [@Robitaille_2019]. 
...
coordinates using the `coords` argument, the `get_geometry` function adds
a simple feature geometry list column (`sfc`) using the {sf} package [@Pebesma_2018].
The geometry method is used by default for all {spatsoc} functions requiring 
coordinates when the `coords` argument is left null. The following examples and
case study will illustrate the geometry interface throughout. 
