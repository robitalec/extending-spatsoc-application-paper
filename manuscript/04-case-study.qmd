...
#| eval: true
#| warning: false
dist_dir_cent <- dist_to_group_centroid(DT, coords, return_rank = TRUE)
dist_dir_cent <- bearing_to_group_centroid(DT, coords)
g1 <- ggplot(DT) +
  geom_histogram(aes(bearing_to_group_centroid), bins = 30) +
...
g <- ggplot(sub, aes(X, Y, color = id)) +
  ...
  geom_text(aes(label = paste0(format(dist_leader, digits = 2),
                               ', ',
                               format(bearing_to_leader, digits = 2),
                               ' rad')), nudge_y = -0.5) +
  geom_point(color = 'black', aes(group_mean_X, group_mean_Y)) +
  geom_abline(slope = slope, intercept = intercept) +
  geom_abline(slope = -1/slope, intercept = intercept_inv,
...
g_dist <- ggplot(DT, aes(CircStats::deg(bearing_to_leader), 
...
az(DT_fogo, coords = c('x_long', 'y_lat'), projection = 4326)
dir_delay_fogo <- dir_corr_delay(DT_fogo, edges_fogo, window = 2)
