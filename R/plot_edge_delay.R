plot_edge_delay <- function(edges, leaders, DT) {
  ...
  sub_leaders <- unique(
    leaders[, .(ID = ID1, mean_direction_delay = round(mean_direction_delay, 2))])[
      order(ID)]

  sub_DT <- DT[timegroup %in% sub_edges$timegroup & ID %in% sub_edges$ID1]

  sub_edges[sub_DT, direction := direction, on = .(ID1 == ID, timegroup == timegroup)]

  left <- sub_edges[ID1 == first(sel_ids)][
    timegroup == median(timegroup),
    .(timegroup = median(seq.int(uniqueN(sub_DT$timegroup))),
      ID1, ID2, direction = round(direction, 2), direction_delay)]
  ...
  g_left <- ggplot() + annotation_custom(
    tableGrob(left, theme = ttheme_default(base_size = font_size), rows = NULL)
  )
  g_right <- ggplot() + annotation_custom(
    tableGrob(right, theme = ttheme_default(base_size = font_size), rows = NULL)
  )
  g_leaders <- ggplot() + annotation_custom(
