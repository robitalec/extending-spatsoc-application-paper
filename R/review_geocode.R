...
review_geocode <- function(DT) {
  ...
  # Manual fixes
  DT_in[grepl('Yakushuma Island, Japan', region),
        region := 'Yakushima, Japan']
  DT_in[grepl('NWT', region),
        region := 'Northwest Territories, Canada']
  DT_in[grepl('Gonarezhou National Park', region),
        region := 'Gonarezhou National Park']
  DT_in[grepl('Gap Arid Zone Research Station', region),
        region := 'NSW 2880, Australia']
  DT_in[grepl('Arctic National Wildlife Refuge', region),
        region := 'Arctic National Wildlife Refuge']
  DT_in[grepl('60.10 N, 148 W', region),
        region := '60.10 N, 148 W']
